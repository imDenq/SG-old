...
import fr.patatedouce.foxysg.utils.board.Board;
import fr.patatedouce.foxysg.utils.board.BoardAdapter;
...
public class BoardLayout implements BoardAdapter {
    ...
    public String getTitle(Player player) {
        String title = FoxySG.getInstance().getConfiguration("scoreboard").getString("title");
    ...
    private List<String> getPlayerScoreboard(Player player) {
        ...
        if (gameManager.getGameState() == GameState.LOBBY) {
            if (PlayerManager.getInstance().getLobbyPlayers().size() < GameManager.getInstance().getMinPlayers()) {
                for (String string : FoxySG.getInstance().getConfiguration("scoreboard").getStringList("lobby-scoreboard")) {
                    board.add(replace(string, player));
                }
            } else if (gameManager.getStartCountdown() != null && !gameManager.getStartCountdown().hasExpired()) {
                for (String string : FoxySG.getInstance().getConfiguration("scoreboard").getStringList("lobby-countdown-scoreboard")) {
                    board.add(replace(string, player));
                }
            }
        }
        if (gameManager.getGameState() == GameState.PREMATCH) {
            for (String string : FoxySG.getInstance().getConfiguration("scoreboard").getStringList("pre-match-scoreboard")) {
                board.add(replace(string, player));
            }
        }
        if (gameManager.getGameState() == GameState.INGAME) {
            if (gameManager.getPvpCountdown() != null && !gameManager.getPvpCountdown().hasExpired()) {
                for (String string : FoxySG.getInstance().getConfiguration("scoreboard").getStringList("pvp-countdown-scoreboard")) {
                ...
                for (String string : FoxySG.getInstance().getConfiguration("scoreboard").getStringList("feast-countdown-scoreboard")) {
                    board.add(borderReplace(replace(string, player)));
                }
            } else if (gameManager.getDeathMatchCountdown() != null && !gameManager.getDeathMatchCountdown().hasExpired()) {
                for (String string : FoxySG.getInstance().getConfiguration("scoreboard").getStringList("deathmatch-countdown-scoreboard")) {
                    board.add(borderReplace(replace(string, player)));
                }
            } else if (gameManager.getDeathMatchCountdown().hasExpired()) {
                for (String string : FoxySG.getInstance().getConfiguration("scoreboard").getStringList("deathmatch-scoreboard")) {
                    board.add(borderReplace(replace(string, player)));
                }
            }
        }
        if (gameManager.getGameState() == GameState.ENDING) {
            for (String string : FoxySG.getInstance().getConfiguration("scoreboard").getStringList("winner-scoreboard")) {
                board.add(replace(string, player));
            }
        }
        return board;
    ...
    public String replace(String string, Player player) {
        ...
        return string
                ...
                .replaceAll("<spectator_chat_boolean>", (data.isSpecChat() ? FoxySG.getInstance().getConfiguration("scoreboard").getString("enabled") : FoxySG.getInstance().getConfiguration("scoreboard").getString("disabled")))
    ...
    public String borderReplace(String string) {
        return string
                .replaceAll("<border_size>", String.valueOf(BorderManager.getInstance().getBorder().getSize()))
