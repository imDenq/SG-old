package fr.patatedouce.foxysg.other;

import fr.patatedouce.foxysg.enums.GameState;
import fr.patatedouce.foxysg.events.*;
import fr.patatedouce.foxysg.managers.*;
import fr.patatedouce.foxysg.utils.*;
import fr.patatedouce.foxysg.utils.chat.Color;
import fr.patatedouce.foxysg.utils.tasks.SplitedRoadProcessor;
import fr.patatedouce.foxysg.FoxySG;
import fr.patatedouce.foxysg.border.Border;
...
import fr.patatedouce.foxysg.utils.countdowns.RebootCountdown;
import org.bukkit.*;
...
public class PlayerListener implements Listener {
    ...
    public void handlePlayerLogin(PlayerLoginEvent event) {
        ...
        if (FoxySG.getInstance().isPluginLoading() || WorldsManager.getInstance().getLobbyWorld() == null) {
            event.disallow(PlayerLoginEvent.Result.KICK_OTHER, fr.patatedouce.foxysg.utils.chat.Color.translate(FoxySG.getInstance().getConfiguration("messages").getString("loading-server"))
                    .replaceAll("<server>", GameManager.getInstance().getServerName())
            );
        }
        if (player.hasPermission("foxysg.join.started")) {
            return;
        }
        if (gameManager.getGameState().equals(GameState.INGAME)) {
            event.disallow(PlayerLoginEvent.Result.KICK_OTHER, fr.patatedouce.foxysg.utils.chat.Color.translate(FoxySG.getInstance().getConfiguration("messages").getString("already-started")));
        } else if (gameManager.getGameState().equals(GameState.ENDING)) {
            event.disallow(PlayerLoginEvent.Result.KICK_OTHER, fr.patatedouce.foxysg.utils.chat.Color.translate(FoxySG.getInstance().getConfiguration("messages").getString("game-end")));
        }
    }
    ...
    public void handleChat(AsyncPlayerChatEvent event) {
        ...
        if (!FoxySG.getInstance().getConfiguration("config").getBoolean("CHAT-FORMAT.ENABLED")) {
            return;
        }

        String format = FoxySG.getInstance().getConfiguration("config").getString("CHAT-FORMAT.FORMAT");
        format = format.replace("<player_display_name>", event.getPlayer().getDisplayName());
        format = format.replace("<message>", event.getMessage());

        if(data.getPoints().getAmount() < 1000) {
            format = format.replace("<prefix>", "§8[§70§8] §7");
            event.setFormat(format);
            return;
        }
        if(data.getPoints().getAmount() >= 1000){
            format = format.replace("<prefix>", "§a✦ ");
            event.setFormat(format);

        }
        if(data.getPoints().getAmount() >= 2000){
            format = format.replace("<prefix>", "§e✬ ");
            event.setFormat(format);
        }
        if(data.getPoints().getAmount() >= 3000){
            format = format.replace("<prefix>", "§6✵ ");
            event.setFormat(format);
        }
        if(data.getPoints().getAmount() >= 5000){
            format = format.replace("<prefix>", "§c❈ ");
            event.setFormat(format);
        }
        if(data.getPoints().getAmount() >= 7500){
            format = format.replace("<prefix>", "§4❉ ");
            event.setFormat(format);
        }
        if(data.getPoints().getAmount() >= 10000){
            format = format.replace("<prefix>", "§d✪ ");
            event.setFormat(format);
        }
        if(data.getPoints().getAmount() >= 50000){
            format = format.replace("<prefix>", "§b§l☬ ");
            event.setFormat(format);
        }
    }
    ...
    public void handleJoin(PlayerJoinEvent event) {
        ...
        if (FoxySG.getInstance().isPluginLoading() || WorldsManager.getInstance().getLobbyWorld() == null) {
            if (!GameManager.getInstance().isToUseLobby()) {
                player.kickPlayer(fr.patatedouce.foxysg.utils.chat.Color.translate(FoxySG.getInstance().getConfiguration("messages").getString("loading-server"))
                        .replaceAll("<server>", GameManager.getInstance().getServerName())
                );
            } else {
                player.sendMessage(fr.patatedouce.foxysg.utils.chat.Color.translate(FoxySG.getInstance().getConfiguration("messages").getString("loading-server"))
                        .replaceAll("<server>", GameManager.getInstance().getServerName())
                );
        ...
        if (GameManager.getInstance().getGameState().equals(GameState.LOBBY)) {
            ...
            FoxySG.getInstance().getConfiguration("messages").getStringList("join-message")
                    .forEach(message -> player.sendMessage(fr.patatedouce.foxysg.utils.chat.Color.translate(message
            ...
            if (GameManager.getInstance().getPrematchCountdown() != null && !GameManager.getInstance().getPrematchCountdown().hasExpired()) {
                player.sendMessage(GameManager.getInstance().getGamePrefix() + fr.patatedouce.foxysg.utils.chat.Color.translate(FoxySG.getInstance().getConfiguration("messages").getString("pre-match-countdown"))
    ...
    public void handleSpectatorChat(AsyncPlayerChatEvent event) {
        ...
        event.setCancelled(true);
        Utils.broadcastMessageToSpectators(FoxySG.getInstance().getConfiguration("config").getString("CHAT-FORMAT.SPECTATOR-FORMAT").replaceAll("<player_display_name>", player.getDisplayName()).replaceAll("<message>", event.getMessage())
        );
    }
    ...
    public void handleBlockBreak(BlockBreakEvent event) {
        ...
        if(PlayerManager.getInstance().isSpectator(player)){
            event.setCancelled(true);
            player.sendMessage(Color.translate("&cVous ne pouvez pas casser de blocks en mode spéctateur!"));
            return;
        }
        PlayerData data = PlayerDataManager.getInstance().getByUUID(player.getUniqueId());

        if (!data.getState().equals(PlayerState.INGAME) && (data.getState().equals(PlayerState.INGAME))) {
            event.setCancelled(true);
            player.sendMessage(fr.patatedouce.foxysg.utils.chat.Color.translate(FoxySG.getInstance().getConfiguration("messages").getString("cannot-break-block")));
        }
    }
    ...
    public void handleBlockPlace(BlockPlaceEvent event) {
        ...
        if (!data.getState().equals(PlayerState.INGAME) && (data.getState().equals(PlayerState.INGAME))){
            event.setCancelled(true);
            player.sendMessage(fr.patatedouce.foxysg.utils.chat.Color.translate(FoxySG.getInstance().getConfiguration("messages").getString("cannot-place-block")));
        }
    }
    ...
    public void handlePortalTeleport(PlayerPortalEvent event) {
        event.setCancelled(true);
        event.getPlayer().sendMessage(fr.patatedouce.foxysg.utils.chat.Color.translate(FoxySG.getInstance().getConfiguration("messages").getString("cannot-enter-portal")));
    }
    ...
    public void handleItemDrop(PlayerDropItemEvent event) {
        ...
        if (!data.getState().equals(PlayerState.INGAME)) {
            event.setCancelled(true);
            player.sendMessage(fr.patatedouce.foxysg.utils.chat.Color.translate(FoxySG.getInstance().getConfiguration("messages").getString("cannot-drop-item")));
        }
    }
    ...
    public void handleChestPlace(BlockPlaceEvent event) {
        ...
        if (!player.hasPermission("foxysg.chests.place")) {
            return;
        }
        if (GameManager.getInstance().getGameState() == GameState.INGAME) {
            return;
        }
        player.sendMessage(fr.patatedouce.foxysg.utils.chat.Color.translate(GameManager.getInstance().getGamePrefix() + "&aVous avez posé un chest: ne posez rien dedans"));
    }
    ...
    public void handleChestBreak(BlockBreakEvent event) {
        ...
        if (event.getBlock().getType().equals(Material.CHEST)) {
            ...
            if (!player.getGameMode().equals(GameMode.CREATIVE)) {
                player.sendMessage(fr.patatedouce.foxysg.utils.chat.Color.translate("&aCreatif requis"));
                return;
            }
            if (!player.isSneaking()) {
                event.setCancelled(true);
                player.sendMessage(GameManager.getInstance().getGamePrefix() + fr.patatedouce.foxysg.utils.chat.Color.translate("&esneak pour casser le chest"));
                return;
            }
            inv.clear();
            player.sendMessage(GameManager.getInstance().getGamePrefix() + fr.patatedouce.foxysg.utils.chat.Color.translate("&echest cassé. items sont cassés"));
        }
    }
    ...
    public void handleCompassInteract(PlayerInteractEvent event) {
        if (event.getAction() != Action.PHYSICAL && event.hasItem() && event.getItem().getType() == Material.COMPASS) {
            ...
            if (targetp != null) {
                ...
                player.sendMessage(fr.patatedouce.foxysg.utils.chat.Color.translate(FoxySG.getInstance().getConfiguration("messages").getString("tracking-player"))
                        .replaceAll("<target>", targetp.getName())
                );
            } else {
                player.sendMessage(fr.patatedouce.foxysg.utils.chat.Color.translate(FoxySG.getInstance().getConfiguration("messages").getString("tracking-player-failed")));
            }
        }
    }
    ...
    public void handleBlockChange(EntityChangeBlockEvent event) {
        ...
        if (block.hasMetadata("fallingBlock")) {
            if (block.getType().equals(Material.CHEST)) {
                block.removeMetadata("fallingBlock", FoxySG.getInstance());
    ...
    public void handleEnderpearl(PlayerInteractEvent event) {
        ...
        if (data.getEnderpearlCooldown() != null && !data.getEnderpearlCooldown().hasExpired()) {
            ...
            player.sendMessage(GameManager.getInstance().getGamePrefix() + fr.patatedouce.foxysg.utils.chat.Color.translate(FoxySG.getInstance().getConfiguration("messages").getString("cannot-use-enderpearl"))
    ...
    public void handleGameDeath(PlayerDeathEvent event) {
        ...
        new BukkitRunnable() {
            public void run() {
                try {
                    ...
                    PlayerManager.getInstance().setSpectating(player);
                    player.sendMessage(fr.patatedouce.foxysg.utils.chat.Color.translate(GameManager.getInstance().getGamePrefix() + FoxySG.getInstance().getConfiguration("messages").getString("become-spectator")));
                } catch (Exception ex) {
                    ex.printStackTrace();
                }
            }
        }.runTaskLater(FoxySG.getInstance(), 2L);
        ...
        if (event.getEntity().getKiller() != null) {
            ...
            killer.sendMessage(fr.patatedouce.foxysg.utils.chat.Color.translate(FoxySG.getInstance().getConfiguration("messages").getString("add-points-because-kill"))
    ...
    public void handleGameWin(SGGameWinEvent event) {
        ...
        player.sendMessage(fr.patatedouce.foxysg.utils.chat.Color.translate(FoxySG.getInstance().getConfiguration("messages").getString("add-points-because-win"))
        ...
        for (String s : FoxySG.getInstance().getConfiguration("messages").getStringList("win-message")) {
        ...
        }.runTaskLater(FoxySG.getInstance(), 20L * 10);
        new BukkitRunnable() {
            public void run() {
        ...
        }.runTaskTimer(FoxySG.getInstance(), 10L, 10L);
        ...
        Bukkit.getConsoleSender().sendMessage(fr.patatedouce.foxysg.utils.chat.Color.translate("&7&m-----------------------------------"));
        Bukkit.getConsoleSender().sendMessage(fr.patatedouce.foxysg.utils.chat.Color.translate("&aSave Success  &c" + size + " data " + (size > 1 ? "files" : "file") + "&a."));
        Bukkit.getConsoleSender().sendMessage(fr.patatedouce.foxysg.utils.chat.Color.translate("&7&m-----------------------------------"));
    }

    @EventHandler
    public void onSGWorldLoad(SGWorldLoad event) {
        if (FoxySG.getInstance().getConfiguration("config").getBoolean("ROADS.MAKE-ROADS")) {
            Bukkit.getConsoleSender().sendMessage(Color.translate("&aRoad processor."));
            new SplitedRoadProcessor(FoxySG.getInstance(), event.getCenterLocation(), 15000, 2).run();
        } else {
            Bukkit.getConsoleSender().sendMessage(Color.translate("&aserveur ouvert aux joueurs !"));
            FoxySG.getInstance().setPluginLoading(false);
        }
    }
    ...
    public void onPlayerTeleportEvent(PlayerTeleportEvent event) {
        ...
        if (x >= size) {
            ...
            player.sendMessage(GameManager.getInstance().getBorderPrefix() + fr.patatedouce.foxysg.utils.chat.Color.translate(FoxySG.getInstance().getConfiguration("messages").getString("stay-in-border")));
            event.setCancelled(true);
        }
        if (z >= size) {
            player.teleport(event.getFrom());
    ...
    public void handleWorldBorderPass(PlayerMoveEvent event) {
        ...
        if (X >= size) {
            ...
            player.sendMessage(GameManager.getInstance().getBorderPrefix()
                    + fr.patatedouce.foxysg.utils.chat.Color.translate(FoxySG.getInstance().getConfiguration("messages").getString("stay-in-border")));
        ...
        if (Z <= -size) {
            player.teleport(event.getFrom());
            player.sendMessage(GameManager.getInstance().getBorderPrefix()
                    + Color.translate(FoxySG.getInstance().getConfiguration("messages").getString("stay-in-border")));
            BorderManager.getInstance().checkBorder(player);
        }
    }
}
