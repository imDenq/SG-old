...
public class BorderTask extends BukkitRunnable {
    ...
    public void run() {
        ...
        if (border.getSeconds() == 0) {
            ...
            Utils.broadcastMessage(GameManager.getInstance().getBorderPrefix() + FoxySG.getInstance().getConfiguration("messages").getString("border-strunk")
        ...
        if (Arrays.asList(5, 4, 3, 2, 1).contains(minutes) && border.getSeconds() == (minutes * 60)) {
            ...
            Utils.broadcastMessage(GameManager.getInstance().getBorderPrefix() + FoxySG.getInstance().getConfiguration("messages").getString("border-strink-minutes")
        ...
        if (Arrays.asList(30, 20, 10, 5, 4, 3, 2, 1).contains(border.getSeconds())) {
            Utils.playSound(Sound.NOTE_PLING);
            Utils.broadcastMessage(GameManager.getInstance().getBorderPrefix() + FoxySG.getInstance().getConfiguration("messages").getString("border-strink-seconds")
                    .replaceAll("<next_border>", String.valueOf(border.getNextBorder()))
