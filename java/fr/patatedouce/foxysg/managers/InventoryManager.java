...
public class InventoryManager {
    ...
    public Inventory getStatsInventory(PlayerData data) {
        Inventory inv = Bukkit.createInventory(null, 9, Color.translate(FoxySG.getInstance().getConfiguration("inventory").getString("stats-inventory.title").replaceAll("<target>", data.getName())));

        ItemBuilder combat = new ItemBuilder(Material.DIAMOND_SWORD);
        combat.setName(FoxySG.getInstance().getConfiguration("inventory").getString("stats-inventory.combat.name"));
        for (String string : FoxySG.getInstance().getConfiguration("inventory").getStringList("stats-inventory.combat.lore")) {
            combat.addLoreLine(replace(string, data));
        }

        ItemBuilder misc = new ItemBuilder(Material.NETHER_STAR);
        misc.setName(FoxySG.getInstance().getConfiguration("inventory").getString("stats-inventory.misc.name"));
        for (String string : FoxySG.getInstance().getConfiguration("inventory").getStringList("stats-inventory.misc.lore")) {
            misc.addLoreLine(replace(string, data));
        }

        ItemBuilder potion = new ItemBuilder(Material.POTION);
        potion.setName(FoxySG.getInstance().getConfiguration("inventory").getString("stats-inventory.potion.name"));
        for (String string : FoxySG.getInstance().getConfiguration("inventory").getStringList("stats-inventory.potion.lore")) {
            potion.addLoreLine(replace(string, data));
        }

        ItemBuilder other = new ItemBuilder(Material.DIAMOND);
        other.setName(FoxySG.getInstance().getConfiguration("inventory").getString("stats-inventory.other.name"));
        for (String string : FoxySG.getInstance().getConfiguration("inventory").getStringList("stats-inventory.other.lore")) {
