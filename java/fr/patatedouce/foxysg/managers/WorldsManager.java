...
import fr.patatedouce.foxysg.events.SGWorldLoad;
...
public class WorldsManager {
    ...
    private void registerWorlds(boolean makeRoads) {
        String lobbyWorldName = FoxySG.getInstance().getConfiguration("config").getString("WORLDS.LOBBY");
        String gameWorldName = FoxySG.getInstance().getConfiguration("config").getString("WORLDS.GAME");
        ...
        Bukkit.getServer().getScheduler().scheduleSyncDelayedTask(FoxySG.getInstance(), () -> {
    ...
    public void saveCurrentWorld() {
        ...
        Bukkit.getServer().getScheduler().scheduleSyncDelayedTask(FoxySG.getInstance(), () -> handleWorldCopy(getGameWorld().getName(), gameWorldName), 20L * 2);
