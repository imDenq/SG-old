...
public class PlayerManager {
    ...
    public void setSpectating(Player player) {
        ...
        player.teleport(GameManager.getInstance().getGameWorldCenterLocation().add(0, 100, 0));

        ItemBuilder alivePlayers = new ItemBuilder(Material.ITEM_FRAME);
        alivePlayers.setNameWithArrows(FoxySG.getInstance().getConfiguration("items").getString("spectator-inventory.alive-players.name"));
        for (String lore : FoxySG.getInstance().getConfiguration("items").getStringList("spectator-inventory.alive-players.lore")) {
            alivePlayers.addLoreLine(lore);
        }

        ItemBuilder lobby = new ItemBuilder(Material.REDSTONE);
        if (GameManager.getInstance().isToUseLobby()) {
            lobby.setNameWithArrows(FoxySG.getInstance().getConfiguration("items").getString("spectator-inventory.quit-sg.lobby-enabled.name"));
            for (String lore : FoxySG.getInstance().getConfiguration("items").getStringList("spectator-inventory.quit-sg.lobby-enabled.lore")) {
                lobby.addLoreLine(lore);
            }
        } else {
            lobby.setNameWithArrows(FoxySG.getInstance().getConfiguration("items").getString("spectator-inventory.quit-sg.lobby-disabled.name"));
            for (String lore : FoxySG.getInstance().getConfiguration("items").getStringList("spectator-inventory.quit-sg.lobby-disabled.lore")) {
                lobby.addLoreLine(lore);
            }
        }

        ItemBuilder inv = new ItemBuilder(Material.BOOK);
        inv.setNameWithArrows(FoxySG.getInstance().getConfiguration("items").getString("spectator-inventory.inspect-inventory.name"));
        for (String lore : FoxySG.getInstance().getConfiguration("items").getStringList("spectator-inventory.inspect-inventory.lore")) {
            inv.addLoreLine(lore);
        }

        ItemBuilder random = new ItemBuilder(Material.WATCH);
        random.setNameWithArrows(FoxySG.getInstance().getConfiguration("items").getString("spectator-inventory.random-teleport.name"));
        for (String lore : FoxySG.getInstance().getConfiguration("items").getStringList("spectator-inventory.random-teleport.lore")) {
