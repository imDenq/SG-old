...
public class CommandFramework implements CommandExecutor {
    ...
    public CommandFramework(FoxySG plugin) {
        this.plugin = plugin;
    ...
    public boolean handleCommand(CommandSender sender, org.bukkit.command.Command cmd, String label, String[] args) {
        for (int i = args.length; i >= 0; i--) {
            ...
            if (commandMap.containsKey(cmdLabel)) {
                ...
                if (command.isAdminOnly() && !sender.hasPermission("foxysg.admin")) {
                    sender.sendMessage(ChatColor.RED + "Syntax erronnée.");
                    return true;
                }
                if (!command.permission().equals("") && (!sender.hasPermission(command.permission()))) {
                    sender.sendMessage(Color.translate("&cPermission issue.")); // TODO change
                    return true;
                }
                if (command.inGameOnly() && !(sender instanceof Player)) {
                    sender.sendMessage(ChatColor.RED + "juste sur le jeu meinkraft.");
                    return true;
                }
        ...
        for (Method m : obj.getClass().getMethods()) {
            if (m.getAnnotation(Command.class) != null) {
                ...
                if (m.getParameterTypes().length > 1 || m.getParameterTypes()[0] != CommandArgs.class) {
                    System.out.println("jpp pas register " + m.getName() + ". (Unexpected method arguments)");
                    continue;
                }
                ...
                if (m.getParameterTypes().length > 1 || m.getParameterTypes().length == 0
                    ...
                    System.out.println(
                            "tab completer issue " + m.getName() + ". (Unexpected method arguments)");
                    continue;
                }
                if (m.getReturnType() != List.class) {
                    System.out.println("tab completer issue " + m.getName() + ". (Unexpected return type)");
                    continue;
                }
    ...
    public void registerHelp() {
        ...
        IndexHelpTopic topic = new IndexHelpTopic(plugin.getName(), "commands pour: " + plugin.getName(), null, help,
                "syntaxe " + plugin.getName() + " commande:");
        ...
        if (map.getCommand(cmdLabel) instanceof BukkitCommand) {
            ...
            try {
                ...
                if (field.get(command) == null) {
                    ...
                    System.out.println("register tab fail " + m.getName()
                            + ". tab already use");
                }
            } catch (Exception ex) {
                ex.printStackTrace();
    ...
    private void defaultCommand(CommandArgs args) {
        args.getSender().sendMessage(args.getLabel() + " failed");
    }
}
