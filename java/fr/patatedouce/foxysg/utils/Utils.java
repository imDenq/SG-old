package fr.patatedouce.foxysg.utils;
...
public class Utils {
    ...
    public static void broadcastMessageToStaff(String message) {
        getOnlinePlayers().stream().filter(player ->
                player.hasPermission("foxysg.staff")).forEach(player -> player.sendMessage(Color.translate(message)));
    }
    ...
    public static void connectPlayer(Player player, String server) {
        ...
        player.sendPluginMessage(FoxySG.getInstance(), "BungeeCord", b.toByteArray());
    }
    ...
    public static void randomPlayer(Player player) {
        ...
        if (online.size() != 0) {
            ...
            player.sendMessage(GameManager.getInstance().getGamePrefix() + Color.translate("&aVous avez été téléporté à &c" + target.getName() + "&a."));
        } else {
            player.sendMessage(GameManager.getInstance().getGamePrefix() + Color.translate("&cAucun joueur en jeu."));
        }
    }
    ...
    public static Inventory createInventoryOther(Player player) {
        Inventory inv = Bukkit.createInventory(null, 54, "Inventaire");
        ...
        for (int i = 36; i <= 49; i++) {
            if (!Arrays.asList(45, 46, 47, 48).contains(i)) {
                inv.setItem(i, new ItemBuilder(Material.STAINED_GLASS_PANE).setDurability(5).setName("&7&o(" + player.getName() + ")").toItemStack());
            }
        }
        return inv;
    }
    ...
    public static void sendGlobalMessage(Player player, String message) {
        ...
        player.sendPluginMessage(FoxySG.getInstance(), "BungeeCord", output.toByteArray());
    }
