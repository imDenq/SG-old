package fr.patatedouce.foxysg.utils.board;
...
public class Board {
	...
	private void init(JavaPlugin plugin) {
		...
		this.objective.setDisplayName(ChatColor.RED.toString() + this.adapter.getTitle(player));

		//(healthName = getOrCreateObjective(scoreboard, "healthName", "health")).setDisplaySlot(DisplaySlot.BELOW_NAME);
		//healthName.setDisplayName(ChatColor.RED + Prestige);
	}
