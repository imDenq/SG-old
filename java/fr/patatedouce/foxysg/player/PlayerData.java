package fr.patatedouce.foxysg.player;
...
import fr.patatedouce.foxysg.utils.Cooldown;
import fr.patatedouce.foxysg.utils.ItemBuilder;
import fr.patatedouce.foxysg.utils.RespawnInfo;
import fr.patatedouce.foxysg.utils.Setting;
...
public class PlayerData {
    ...
    public PlayerData(UUID uuid) {
        ...
        settings.add(new Setting("ChestClick", Material.CHEST, true, 0, ""));
        settings.add(new Setting("GlassBorder", Material.STAINED_GLASS, true, 14,0,""));
        settings.add(new Setting("Scoreboard", Material.ITEM_FRAME, true, 0,""));
        settings.add(new Setting("Lightning-On-Kill", Material.SKULL_ITEM, true, 1, 0,""));
        settings.add(new Setting("Chest-Auto-Pickup", Material.ENDER_CHEST, false, 50, ""));
    ...
    public Inventory getSettingsInventory() {
        Inventory inv = Bukkit.createInventory(null, FoxySG.getInstance().getConfiguration("inventory").getInt("settings-inventory.size"), Color.translate(FoxySG.getInstance().getConfiguration("inventory").getString("settings-inventory.title")));
        ...
        settings.forEach(setting -> {
            ...
            item.setName(FoxySG.getInstance().getConfiguration("inventory").getString("settings-inventory.name").replaceAll("<name>", setting.getName()));

            for (String string : FoxySG.getInstance().getConfiguration("inventory").getStringList("settings-inventory.settings-line")) {
                item.addLoreLine(string);
            }

            if (setting.getDescription().length > 0) {
                if (setting.getName().equals("ChestClick")) {
                    for (String description : FoxySG.getInstance().getConfiguration("inventory").getStringList("settings-inventory.description.chestclick")) {
                ...
                if (setting.getName().equals("GlassBorder")) {
                    for (String description : FoxySG.getInstance().getConfiguration("inventory").getStringList("settings-inventory.description.glass-border")) {
                        item.addLoreLine(description);
                    }
                    settingFiller(setting, item);
                ...
                if (setting.getName().equals("Scoreboard")) {
                    for (String description : FoxySG.getInstance().getConfiguration("inventory").getStringList("settings-inventory.description.scoreboard")) {
                        item.addLoreLine(description);
                    }
                    settingFiller(setting, item);
                ...
                if (setting.getName().equals("Lightning-On-Kill")) {
                    for (String description : FoxySG.getInstance().getConfiguration("inventory").getStringList("settings-inventory.description.lightning-on-kill")) {
                        item.addLoreLine(description);
                    }
                    settingFiller(setting, item);
                ...
                if (setting.getName().equals("Chest-Auto-Pickup")) {
                    for (String description : FoxySG.getInstance().getConfiguration("inventory").getStringList("settings-inventory.description.chest-auto-pickup")) {
                        item.addLoreLine(description);
                    }
    ...
    private void settingFiller(Setting setting, ItemBuilder item) {
        for (String string : FoxySG.getInstance().getConfiguration("inventory").getStringList("settings-inventory.info-status")) {
            item.addLoreLine(string
                    .replaceAll("<name>", String.valueOf(setting.getName()))
                    .replaceAll("<settings_status>", (setting.isEnabled() ? FoxySG.getInstance().getConfiguration("inventory").getString("enabled") : FoxySG.getInstance().getConfiguration("inventory").getString("disabled")))
            );
        }
        for (String string : FoxySG.getInstance().getConfiguration("inventory").getStringList("settings-inventory.settings-line")) {
            item.addLoreLine(string);
    ...
    private void settingWithPointsFiller(Setting setting, ItemBuilder item) {
        ...
        if (getPoints().getAmount() < setting.getRequiredPoints()) {
            for (String string : FoxySG.getInstance().getConfiguration("inventory").getStringList("settings-inventory.not-enough-points-lore")) {
                item.addLoreLine(string
